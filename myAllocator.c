...
BlockPrefix_t *findNextFit(size_t s){
    ...
    while (p){
        if (!p->allocated && computeUsableSpace(p) >= s){
            lastAllocated = getNextPrefix(p);
            return p;
        }
        p = getNextPrefix(p);
    }
    while(p != lastAllocated){
        if (!p->allocated && computeUsableSpace(p) >= s){
            lastAllocated = getNextPrefix(p);
            return p;
        }
...
void *resizeRegion(void *r, size_t newSize){
    ...
    if (oldSize >= newSize)	/* old region is big enough */
        ...
        size_t tlength = (void *)(next->suffix)-r;
        if(next->allocated == 0 && tlength >= newSize){
            if(tlength >= newSize + prefixSize + suffixSize + 8){//split the region
                size_t asize = align8(newSize);
                void *freeSliverStart = (void *)p + prefixSize + suffixSize + asize;
                void *freeSliverEnd = computeNextPrefixAddr(next);
                makeFreeBlock(freeSliverStart, freeSliverEnd - freeSliverStart);
                BlockSuffix_t *s = freeSliverStart-suffixSize;
                s->prefix = p;
                p->suffix = s;
                return p;
            }
